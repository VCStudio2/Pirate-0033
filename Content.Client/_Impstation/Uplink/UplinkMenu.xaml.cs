...
public sealed partial class UplinkMenu : FancyWindow
{
    ...
    private List<ListingDataWithCostModifiers> _cachedListings = new(); // Pirate: use current store price modifiers.
    ...
    private void AddListingGui(ListingDataWithCostModifiers listing)
    {
        ...
        var newListing = new UplinkMarketListing(listing, listingInStock, discount, hasBalance, Owner, texture);
    ...
    private string GetDiscountString(ListingDataWithCostModifiers listing)
    {
        ...
        {
            foreach (var amount in modifiers.Values)
                return Loc.GetString("store-ui-discount-display", ("amount", amount.ToString("P0")));
        }

        var text = new StringBuilder("(");
        foreach (var (currency, amount) in modifiers)
        {
            if (text.Length > 1)
                text.Append(", ");

            var currencyPrototype = _prototypeManager.Index(currency);
            text.Append(Loc.GetString("store-ui-discount-display-with-currency",
